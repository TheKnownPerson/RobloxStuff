...
local function AimbotLoader()
	...
	local oldcam = nil
	local smoothness = true -- optional toggle
	local lastVictimUpdate = 0
	...
	local function getTeamCount()
		...
		return #teamservice:GetChildren()
	end
			...
			if victim:IsA("Model")
				...
				if victimPlayer and victimPlayer ~= player then
					if teamCount < 2 or (victimPlayer.Team ~= player.Team) then
						local screenpos, onScreen = camera:WorldToViewportPoint(victim.HumanoidRootPart.Position)
						if onScreen then
	...
	local function aimAtTarget(target)
		if target and target:FindFirstChild("HumanoidRootPart") then
			...
			-- Default to head unless otherwise specified
			if HeadPart and HeadPart.BackgroundColor3 == Color3.fromRGB(90, 90, 90) then
				if target:FindFirstChild("Head") then
					position = target.Head.Position
					radius = 35
				end
			elseif TorsoPart and TorsoPart.BackgroundColor3 == Color3.fromRGB(90, 90, 90) then
	...
	rs.RenderStepped:Connect(function()
		if aimbot and isRightMouseDown then
			local now = tick()
			if now - lastVictimUpdate >= 0.1 then
				cachedVictim = findvictim()
				lastVictimUpdate = now
			end

			if cachedVictim then
				aimAtTarget(cachedVictim)
			end
		end
	end)
	...
	coroutine.wrap(function()
		while true do
			...
			if not BGui:FindFirstChild("ESP") then
				if aimbot or isRightMouseDown then
					aimbot = false
					isRightMouseDown = false
					print("Disabled aimbot due to missing ESP.")
				end
			end
		end
	end)()
